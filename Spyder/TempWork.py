# -*- coding: utf-8 -*-
"""
Created on Fri Jan 27 15:57:04 2017
...
inputFile = "SVN_19K_Geocoded.xlsx"
countryPostCodeRegex ="^([0-9]+)$|^$"


xls_file = pd.ExcelFile(dir+inputFile)
df = xls_file.parse('Dnb_MC',converters={'Postcode': str,'Street':str})
...
df['key'] = df['City'] + "_" + df['Postcode']
o = df.copy(deep=True)

#6 Sample rows (probably based on postcode).
df = sampleRowsOnPostCode('key',10000)
z[['Street','Postcode','City', 'SL', 'Source']].to_csv(dir+'SVN_Forward_Testfile_'+inputFile ,index=False,header=True ,encoding="UTF-8",sep=";")
